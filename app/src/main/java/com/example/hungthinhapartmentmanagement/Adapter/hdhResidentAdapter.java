...
import com.example.hungthinhapartmentmanagement.Activity.hdhEditResidentActivity;
...
public class hdhResidentAdapter extends RecyclerView.Adapter<hdhResidentAdapter.ResidentViewHolder> {
    private List<Residents> residentsList;
    private Context context;
    private FirebaseFirestore db;
    private FirebaseAuth mAuth;

    public hdhResidentAdapter(Context context, List<Residents> residentsList) {
    ...
    public void onBindViewHolder(ResidentViewHolder holder, int position) {
        Residents residents = residentsList.get(position);
        holder.tvApartmentNumber.setText(residents.getApartmentId());
        holder.tvFullName.setText(residents.getFullName());
        holder.tvGender.setText(residents.getGender());
        holder.tvPhone.setText(residents.getPhone());
        holder.tvBirthday.setText(residents.getBirthday());
        holder.tvEmail.setText(residents.getEmail());
        holder.tvRelationship.setText(residents.isRelationship() ? "Có" : "Không");

        // Xử lý nút Edit
        holder.btnEdit.setOnClickListener(v -> {
            Intent intent = new Intent(context, hdhEditResidentActivity.class);
            intent.putExtra("residentId", residentsList.get(position).getDocumentId()); // Giả định Residents có getter cho documentId
        ...
        holder.btnDelete.setOnClickListener(v -> {
            new AlertDialog.Builder(context)
                    ...
                    .setMessage("Bạn có muốn xóa cư dân " + residents.getFullName() + " không?")
                    .setPositiveButton("Có", (dialog, which) -> {
                        int currentPosition = holder.getAdapterPosition();
                        if (currentPosition != RecyclerView.NO_POSITION) {
                            deleteResident(residents.getDocumentId(), currentPosition);
                        }
                    })
